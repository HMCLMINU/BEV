...
class MapMaskGenerator:
    ...
    def ego_trajectory_mask(self, agent: carla.Actor, trajectory_arr: carla.Location) -> Mask:
        canvas = self.make_empty_mask()
        tmp_location = copy.copy(trajectory_arr[2:]) # except first zero waypoints
        x_pose = agent.get_location().x
        y_pose = agent.get_location().y
        if self._old_nearest_point_index is None:
            # search nearest point index
            dx = [x_pose - point.x for point in tmp_location]
            dy = [y_pose - point.y for point in tmp_location]
            d = np.hypot(dx, dy)
            nearest_ind = np.argmin(d)
            self._old_nearest_point_index = nearest_ind
        else:
            ind = self._old_nearest_point_index
            distance_this_index = self.calc_distance(x_pose, y_pose, tmp_location[ind])
            while True:
                distance_next_index = self.calc_distance(x_pose, y_pose, tmp_location[ind + 1])
                if distance_this_index < distance_next_index:
                    break
                ind = ind + 1 if (ind + 1) < len(tmp_location) else ind
                distance_this_index = distance_next_index
            self._old_nearest_point_index = ind

        # print("{}".format(nearest_ind))
        if (self._old_nearest_point_index == len(tmp_location)-1):
            tmp_location = None
        else:
            tmp_location = tmp_location[self._old_nearest_point_index:]
            tmp_location = [self.location_to_pixel(loc) for loc in tmp_location]
        cv.fillPoly(img=canvas, pts=np.int32([tmp_location]), color=COLOR_ON)
        return canvas

    # def ego_past_trajectory_mask(self, agent: carla.Actor, trajectory_arr: carla.Location) -> Mask:
    #     canvas = self.make_empty_mask()
    #     past_location = copy.copy(trajectory_arr[2:]) # except first zero waypoints

    #     past_location = past_location[0:self._old_nearest_point_index-1]
    #     print("{}".format(len(past_location)))
    #     past_location = [self.location_to_pixel(loc) for loc in past_location]
        
    #     cv.fillPoly(img=canvas, pts=np.int32([past_location]), color=COLOR_ON)
    #     return canvas

    def calc_distance(self, ego_x, ego_y, point):
        dx = ego_x - point.x
        dy = ego_y - point.y
