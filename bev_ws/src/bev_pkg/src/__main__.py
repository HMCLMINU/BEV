...
class carla_ros():
    ...
    def tarjCallback(self, msg):
        ...
        for marker in msg.markers:
            ...
            self.pose_arr.header.frame_id = str(i)
            self.pose_arr.poses.append(pose_msg) 
            self.traj.append(carla.Location(pose_msg.position.x, -pose_msg.position.y, 0.0))
            i+=1
    ...
    def main(self):
        ...
        while not rospy.is_shutdown():
            ...
            # if count == 0:
            #     for loc in self.traj:
                    
            #         if i < 50:
            #             print("{}".format(loc))
            #         i += 1
            
            birdview: BirdView = birdview_producer.produce(agent_vehicle=agent, trajectory=self.traj, count=count)
