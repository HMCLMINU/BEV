...
DEFAULT_HEIGHT = 480  # its 80m when density is 6px/m
DEFAULT_WIDTH = 360  # its 60m when density is 6px/m
...
class BirdViewProducer:
    ...
    def __init__(self) -> None:
        ...
        PixelDimensions.width = DEFAULT_WIDTH
        PixelDimensions.height = DEFAULT_HEIGHT
        render_lanes_on_junctions = FALSE
        self.target_size = PixelDimensions
        self.pixels_per_meter = 6
        self._crop_type = BirdViewCropType.FRONT_AND_REAR_AREA

        if self._crop_type is BirdViewCropType.FRONT_AND_REAR_AREA:
            rendering_square_size = round(
                square_fitting_rect_at_any_rotation(self.target_size)
            )
        elif self._crop_type is BirdViewCropType.FRONT_AREA_ONLY:
            # We must keep rendering size from FRONT_AND_REAR_AREA (in order to avoid rotation issues)
            enlarged_size = PixelDimensions(
                width=self.target_size.width, height=self.target_size.height * 2
            )
            rendering_square_size = round(
